...
# 3. 跑起来
bun run packages/cli/bin/nib "在 README 末尾加一行 Hello"
```

首次运行会进入 Ink TUI：消息流 + 工具调用着色 + 底部实时统计（step / tokens / 成本）。
每次工具调用前都会请求审批（`--yes` 一键自动通过）。

## CLI

```bash
nib "<prompt>"                  # 默认带 read_file/write_file/bash/glob 跑 ReAct loop
nib --yes "<prompt>"            # 自动通过所有工具调用（DANGEROUS）
nib --max-steps N "<prompt>"    # 覆盖步数上限（默认 10）
...
│   │   │   └── index.ts
│   │   └── test/            # 77 单测（含安全策略 / 超时 / 审批 / 限额）
│   ├── cli/                 # ✅ Ink TUI
│   │   ├── src/
│   │   │   ├── components/  # App + Footer（实时 step/tokens/成本）
│   │   │   ├── commands/    # ⏳ /plan /test /review /commit /clear (Phase 4)
